...
Item {
    ...
    readonly property string cacheDir: Quickshell.env("HOME") + "/.cache/quickshell-mangowm"
    readonly property string configDir: forkDir
    ...
    Process {
        id: reloadShellProc
        command: ["sh", "-c", "pkill -f 'qs .*quickshell-mangowm/bar/shell.qml' 2>/dev/null || true; qs -p \"$1/bar/shell.qml\" >/dev/null 2>&1 &", "--", root.forkDir]
    }
    ...
    Process {
        id: gameModeProc
        command: [root.configDir + "/scripts/gamemode.sh"]
        running: false
    }
    ...
    FileView {
        ...
        path: root.configDir + "/scripts/gamemode.sh"
        watchChanges: true
        onFileChanged: reload()
    }
    ...
    FileView {
        id: shellFile
        path: root.configDir + "/bar/shell.qml"
        watchChanges: true
        onFileChanged: reload()
    }
    ...
    FileView {
        id: readmeFile
        path: root.configDir + "/bar/README.md"
        watchChanges: true
        onFileChanged: reload()
    }
        ...
        Flickable {
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    ActionButton {
                        label: "System"
                        description: "Open minimal system panel"
                        accentColor: Colors.purple
                        onPressed: root.rootWindow.openSurface("system")
                    }
                    ActionButton {
                        label: "Wi-Fi"
                ...
                ColumnLayout {
                    ...
                    InfoRow { title: "Super + Space"; detail: "Suggested: open app launcher" }
                    InfoRow { title: "Super + C"; detail: "Suggested: open wallpaper picker" }
                    InfoRow { title: "Super + V"; detail: "Suggested: open clipboard history" }
                    InfoRow { title: "Super + W"; detail: "Suggested: open Wi-Fi surface" }
                    InfoRow { title: "Super + X"; detail: "Suggested: open system panel" }
                    InfoRow { title: "Super + O"; detail: "Suggested: open power surface" }
                    InfoRow { title: "Note"; detail: "Bind these in MangoWM using its own keybinding system." }
                }
                ...
                ColumnLayout {
                    ...
                    InfoRow { title: "Fork config dir"; detail: root.configDir }
                    InfoRow { title: "Shell entrypoint"; detail: shellFile.path + " • " + root.fileStatus(shellFile) }
                    InfoRow { title: "Colors file"; detail: colorsFile.path + " • " + root.fileStatus(colorsFile) }
                    InfoRow { title: "Game mode script"; detail: gameModeScriptFile.path + " • " + root.fileStatus(gameModeScriptFile) }
                    InfoRow { title: "Fork README"; detail: readmeFile.path + " • " + root.fileStatus(readmeFile) }
                    ...
                    ActionButton {
                        label: "Open MangoWM note"
                        description: "This fork still needs compositor-specific MangoWM hooks where desired."
                        accentColor: Colors.blue
                        onPressed: root.currentTab = 2
                    }
                }
            }
        }
